...
export const renderChildren = ({ formSpec, components, idMapper, onChange, data }, formData) => {
    ...
    return orderedFields.map(fieldSpec => {
...
export const ListInput = (props) => {
    const { name, value = [], fieldSpec, onChange, formData = {} } = props;
    ...
    const listIdMapper = (childName) => `${name}-${childName}`;
    const children = renderChildren({ ...props, idMapper: props.idMapper || listIdMapper })
    return (
        <div>
            {children}

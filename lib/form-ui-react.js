...
export const useForm = (formSpec, initialData, formName) => {
...
export const FormReact = (props) => {
    ...
    const { form, data: formData, setData: setFormData, errors } = useForm(props.formSpec, props.data, props.formName);
    ...
    const changeHandler = event => {
        const newFormData = form.handleChange(event);
    ...
    const submitHandler = (event) => {
        const isFormValid = form.handleSubmit(event);
        return isFormValid && props?.onSubmit(formData);
    }
    return (
        <form onSubmit={submitHandler}>
            <Paginator form={form} components={props.components} idMapper={props.idMapper} onChange={changeHandler}>
            </Paginator>
        </form>
    );
};

...
export const useForm = (formSpec, components, data, formName, idMapper) => {
    let form = Form.get(formName) ?? new Form(formSpec, { name: formName, data, components, idMapper });
    ...
    if (lastFormSpec !== formSpec) {
        ...
        Form.clear(formName);
        form = new Form(formSpec, { name: formName, data, components, idMapper });
    ...
    //     setReactData(data);
    // };
    return { form, data: formData, setData: setFormData, errors: form.errors };
}

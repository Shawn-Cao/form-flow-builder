...
// Deep merge object fields (1 level). Note: array items should be replaced, custom behavior can be handled by hosting component
export const merge = (target, source) => {
    ...
    for (const key of Object.keys(source)) {
        if (source[key] instanceof Object && !Array.isArray(source[key])) Object.assign(source[key], merge(target[key], source[key]))
    }
...
export const isValueEvent = (evt => evt.target && evt.target.name && evt.target.value && evt.preventDefault);
